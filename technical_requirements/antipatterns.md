...
- Добавлен критический антипаттерн "Теория вместо практики"
- Уроки из реального инцидента с переключением Feed
...
## 🚫 Топ-11 антипаттернов TDD

### 1. ❌ "Теория вместо практики" (КРИТИЧЕСКИЙ)

**Как выглядит:**
```
// Анализирую код
"Логика корректная, должно работать"
"Код выглядит правильно"
"Теоретически все ОК"
// НЕ запускаю приложение
// НЕ делаю скриншоты
// НЕ проверяю логи
```

**Почему это плохо:**
- Код без проверки = несуществующий функционал
- Race conditions не видны в статическом анализе
- UI может вести себя не так, как ожидается
- Пользователь получает нерабочий функционал

**Как правильно:**
```swift
// 1. Написать код с логированием
init() {
    ComprehensiveLogger.shared.log(.ui, .info, "View initialized")
}

// 2. Запустить приложение
// 3. Сделать скриншот до
// 4. Выполнить действие
// 5. Сделать скриншот после
// 6. Проверить логи
// 7. Написать и запустить UI тест
// 8. Только после всех проверок - "работает"
```

### 2. ❌ "Напишу тесты потом"
