...
struct NotificationListView: View {
    ...
    var body: some View {
        NavigationView {
                            ...
                            NotificationRow(notification: notification)
                                ...
                                .swipeActions(edge: .leading, allowsFullSwipe: true) {
                                    if !notification.isRead {
                                        Button {
                                            Task {
                                                try? await notificationService.markAsRead(notification)
                                            }
                                        } label: {
            ...
            .onReceive(notificationService.notifications) { newNotifications in
...
struct NotificationRow: View {
    ...
    private func destinationView() -> some View {
        ...
        case .courseAssigned:
            LearningListView()
        case .testReminder, .testAvailable, .testDeadline:
            TestListView()
        case .certificateEarned, .certificateIssued:
            CertificateListView()
        case .onboardingTask:
